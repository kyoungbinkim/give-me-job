...
- 출처: STAFFS STORY, "취업 자소서 쓰는법: 합격률 높이는 꿀팁 7가지", 네이버 블로그, 2024-07-30.
...
3. 인재상과 JD를 분석한다.
4. `resume.md`에서 사용할 경험 후보를 고른다.
5. 문항별 자기소개서 초안을 작성한다.
6. `Evidence Map`으로 문장과 근거 경험을 연결한다.
7. HR 리뷰와 제출 전 체크리스트를 실행한다.
8. 사용자가 승인하면 최종본을 저장한다.
9. 사이트별 제출 방식에 따라 수동 제출 가이드, 이메일 draft, 브라우저 자동 입력 중 하나를 선택한다.
